/* ==========================================================================
   ...
   --------------------------------------------------------------------------
   The single source of truth for events.html and event.html. Add, edit or
   remove an event here — no HTML changes needed. Order in this file does not
   matter: the pages sort and classify events from their dates.

   HOW TO ADD AN EVENT
     Copy an existing object, give it a unique `id` and `slug` (lower-case,
     hyphens only; the slug becomes event.html?e=<slug>), and fill in what you
     know. Anything unknown: leave it as null (or [] for lists) — the pages
     show "To be announced" for missing upcoming-event details and simply hide
     missing fields on past events. Never guess a date, venue or price.

   FIELDS
     id, slug          unique identifiers
     title             event name
     tagline           optional one-line hook (upcoming events)
     status            'upcoming' | 'live' | 'past'. Used only when the event has
                       no `date`. When `date` is set the status is computed from
                       the dates (see assets/js/events.js -> resolveStatus), so an
                       event moves from Upcoming to Past by itself — Asia/Kolkata
                       time, end of `endDate` (or `date`).
     date, endDate     'YYYY-MM-DD' (endDate for multi-day events) or null
     time              free text, e.g. '6:30 PM IST', or null
     datePublished     'YYYY-MM-DD' — when the archive entry was published. Only a
                       fallback for sorting/labels; NOT the event date.
     type              e.g. 'Movie Screening', 'Online Talk', 'Workshop'
     series, session, topic   optional series name / session number / session theme
     ...
     organizer         only when the event was organised/presented by someone else
     organizerLabel    optional label for it (default 'Organised by'; e.g. 'Presented by')
     dateNote          optional footnote shown under the date (where it came from, etc.)
     entry             free text on fee / registration, or null
     images            [{ src, alt, width?, height? }] — first image is the cover.
                       Local files live in assets/images/events/. Images hosted on
                       aniradichita.wordpress.com are resized on request (?w=...).
     videos            [{ provider: 'youtube' | 'videopress', id, title }]
     sourceUrl         original page (archive) — shown as "Original post"
     ticketing         null, or the block described below
     featured          true = shown as a large feature in the Upcoming section

   TICKETING (upcoming events)
     ticketing: {
       enabled: true,
       provider: 'razorpay',
       checkoutUrl: '',        // <-- paste the Razorpay payment-page URL here
       buttonLabel: 'Book Tickets',
       pendingLabel: 'Tickets Releasing Soon',
       opensAt: null,          // optional ISO time, e.g. '2026-09-22T18:00:00+05:30'
       info: [{ label, value }] // rows for the "Ticket information" panel
     }
     While checkoutUrl is empty (or opensAt is in the future) the page shows a
     disabled "Tickets Releasing Soon" button. Paste an https URL and the button
     turns into a live "Book Tickets" link — nothing else to change.
     (Razorpay Payment Pages look like https://rzp.io/l/xxxxxxx or
     https://pages.razorpay.com/xxxxxxx.) All the logic is in assets/js/ticketing.js.

   Archive entries were imported from https://aniradichita.wordpress.com/portfolio/
   plus event-like pages/posts on that site. Details come from the source text;
   fields the source does not state are left null.
   ========================================================================== */
window.ATFA_EVENTS = [
  {
    ...
    longDescription: [
      ...
      'Tickets are releasing soon. Stay tuned for the venue, time and booking details.'
    ],
    venue: null,      // not announced yet — set when known
    location: null,   // city / area, when known
    speakers: [],
    collaborators: [],
    ...
    images: [
      {
        src: 'assets/images/events/sunday-decodes.jpg',
    ...
    ticketing: {
      enabled: true,
      provider: 'razorpay',
      checkoutUrl: '',          // paste the Razorpay payment-page URL here when it is ready
      buttonLabel: 'Book Tickets',
      pendingLabel: 'Tickets Releasing Soon',
      opensAt: null,
      info: [
        { label: 'Ticket release', value: 'Releasing soon' },
        { label: 'Price', value: null },
        { label: 'How to book', value: 'The Book Tickets button on this page goes live when tickets are released.' }
      ]
    }
  },
  /* ---- Archive (past events) ---- */
